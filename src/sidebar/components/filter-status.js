 ...
 *   [by (<focusedUser>|everybody)] [(and <forcedCount> more)]`
 */
export default function FilterStatus() {
  ...
  // In all states except for inactive 'focus' mode, the count of annotations
  // is rendered as `(<resultCount>|No) (result[s]|annotation[s])`
  let countFacet = (
  ...
  if (filterMode === 'focus' && !filterState.focusActive) {
    countFacet = <span className="filter-facet--pre">everybody</span>;
  }
    ...
    <div className="filter-status">
      <div className="u-layout-row--align-center">
        <div className="filter-status__text">
          {visibleCount > 0 && <span>Showing</span>} {countFacet}{' '}
          {filterMode === 'query' && (
            <span>
              for{' '}
