 ...
 * @prop {string} [href] - URL of the external link to open when this item is clicked. Either the `href` or an  `onClick` callback should be supplied.
 * @prop {string} [iconAlt] - Alt text for icon.
 * @prop {string} [icon] - Name or URL of icon to display. If the value is a URL it is displayed using an `<img>`, if it is a name it is displayed using `SvgIcon`.  If the property is `"blank"` a blank placeholder is displayed in place of an icon. If the property is falsey, no placeholder is displayed. The placeholder is useful to keep menu item labels aligned in a list if some items have icons and others do not.
 * @prop {boolean} [isDisabled] - Dim the label to indicate that this item is not currently available.  The `onClick` callback will still be invoked when this item is clicked and the submenu, if any, can still be toggled.
 * @prop {boolean} [isExpanded] - Indicates that the submenu associated with this item is currently open.
 * @prop {boolean} [isSelected] - Display an indicator to show that this menu item represents something which is currently selected/active/focused.
 * @prop {boolean} [isSubmenuItem] - True if this item is part of a submenu, in which case it is rendered with a different style (shaded background)
 * @prop {boolean} [isSubmenuVisible] - If present, display a button to toggle the sub-menu associated with this item and indicate the current state; `true` if the submenu is visible. Note. Omit this prop, or set it to null, if there is no `submenu`.
 * @prop {string} label - Label of the menu item.
 * @prop {(e: Event) => any} [onClick] - Callback to invoke when the menu item is clicked.
 * @prop {(e: Event) => any} [onToggleSubmenu] - Callback when the user clicks on the toggle to change the expanded state of the menu.
 * @prop {Object} [submenu] - Contents of the submenu for this item.  This is typically a list of `MenuItem` components with the `isSubmenuItem` prop set to `true`, but can include other content as well. The submenu is only rendered if `isSubmenuVisible` is `true`.
 */
