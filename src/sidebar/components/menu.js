...
/**
 * @typedef MenuProps
 * @prop {Object} [align] - Whether the menu content is aligned with the left (default) or right edges of the toggle element.
 * @prop {string} [arrowClass] - Additional CSS class for the arrow caret at the edge of the menu content that "points" toward the menu's toggle button. This can be used to adjust the position of that caret respective to the toggle button.
 * @prop {Object} [label] - Label element for the toggle button that hides and shows the menu.
 * @prop {Object} [children] - Menu items and sections to display in the content area of the menu.  These are typically `MenuSection` and `MenuItem` components, but other custom content is also allowed.
 * @prop {boolean} [containerPositioned] - Whether the menu elements should be positioned relative to the Menu container. When `false`, the consumer is responsible for positioning.
 ...
 * @prop {(open: boolean) => any} [onOpenChanged] - Callback invoked when the menu is opened or closed.  This can be used, for example, to reset any ephemeral state that the menu content may have.
 * @prop {string} title - A title for the menu. This is important for accessibility if the menu's toggle button has only an icon as a label.
 * @prop {boolean} [menuIndicator] - Whether to display an indicator next to the label that there is a dropdown menu.
 */
